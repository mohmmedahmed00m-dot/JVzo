...
export function ExportPage() {
  ...
  const triggerDownload = (exportId: string) => {
    // Fetch as blob with the bearer token, then download.
    fetch(api.downloadUrl(exportId), { headers: { Authorization: `Bearer ${getAccessToken()}` }, credentials: 'include' })
      .then((res) => res.blob())
      .then((blob) => {
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url; a.download = `export-${exportId.slice(0, 8)}.zip`;
        a.click(); URL.revokeObjectURL(url);
      })
      .catch(() => toast.push('error', 'Download failed'));
  };
